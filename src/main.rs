...
fn main() {
    ...
    .cloned()
    .cycle();
    let mut mesurs = Mesurs::default();
    mesurs.inst = Some(Instant::now());
    let mesurs = Arc::new(Mutex::new(mesurs));
    ...
    loop {
        ...
        let mut v = [0u8; 4 * 12];
        match port.read_exact(&mut v) {
            Ok(_) => {
                let mut m = mesurs.lock().unwrap();
                let v: Vec<f32> = v
                    .chunks_exact(4)
                    .map(|chunk| f32::from_le_bytes(chunk.try_into().expect(&format!("{:?}", chunk))))
                    .collect();
                // println!("{:?}", v);
                let (f, s) = v.split_at(6);
                println!("imu1 {:?}", f);
                println!("imu2 {:?}", s);
